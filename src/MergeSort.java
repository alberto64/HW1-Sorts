...
public class MergeSort {
	...
	private static void mergeSortHelper(int[] mergeList, int lowerIdx, int higherIdx) {
	 ...
	 static void merge(int[] mergeList, int lowerIdx, int middleIdx, int higherIdx) 
	    { 
	...
	private static void merge(int[] mergeList, int lowerIdx, int higherIdx) {
		int[] tempArray = new int[higherIdx-lowerIdx+1];
		int tempIdx = 0;
		int left = lowerIdx, right = higherIdx;
		while(left < right) {
			while(mergeList[left] < mergeList[right] && left < right) {
				tempArray[tempIdx++] = mergeList[left++];
			}
			while(mergeList[right] < mergeList[left]  && left < right) {
				tempArray[tempIdx++] = mergeList[right--];
			}
		}
		for(int idx = 0; idx < tempArray.length; idx++) {
			mergeList[idx+lowerIdx] = tempArray[idx];
		}
	}
